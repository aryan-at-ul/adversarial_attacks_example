...
def read_one_image(image_path):
    ...
    # image_pp = preprocess(image_np)[np.newaxis, :, :, :]
    # # image_tensor = transform(image).unsqueeze(0).to(device)
    # image_pp = torch.from_numpy(image_pp).to(device)
    image_np = np.array(image)
    image_pp = preprocess(image_np).unsqueeze(0)
    return image_pp
...
def make_adversarial_example(image_pp, epsilon, dirs_names,class_name,data_typ,image_name,true_labels_tensor):
    ...
    adversarial_image_np = np.squeeze(adversarial_image_np)
    adversarial_image_np = np.transpose(adversarial_image_np, (1, 2, 0))
    adversarial_image_np = (adversarial_image_np * std) + mean
    adversarial_image_np = (adversarial_image_np * 255).clip(0, 255).astype(np.uint8)

    adversarial_image = Image.fromarray(adversarial_image_np)
...
def run_for_one_folder(folder_path):
    ...
    for one in folder_path:
        ...
        true_label = 0 if class_name == "NORMAL" else 1
        true_labels_tensor = torch.tensor([true_label], dtype=torch.long, device=device)

        # print(f"processing {class_name} {data_typ} {image_name} ---> {one}")
        image = read_one_image(one)
        epsilon = 0.03  # Choose a value for epsilon (e.g., 0.03)
        perturbed_image = make_adversarial_example(image, epsilon, dirs_names,class_name,data_typ,image_name,true_labels_tensor)
...
def test_run():
    print("starting")
    make_dirs(current_file_path+'/'+'chest_xray_fgsm')
